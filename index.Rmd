...
Let's remove uninformative(?) words
...
# Comments on the Forecasting Exercise

The problem can be cast as   

$$y_t = f(X_{t-1})+\varepsilon_t$$
where $y_t$ is the direction of change in the interest rate (e.g., $y_t\in\{-1,0,1\}$) rate $X_{t-1}$ is a vactor of predetermined features.  

In our case, $X_{t-1}$ includes a battary of text-related indicators, as well as other economic \& financial variables and __interactions__.  

The goal of the exercise should be defined in terms of **prediction quality** (RMSE is not appropriate here.)  

---

# Comments on the Forecasting Exercise

Lasso seems extremely fit to answer tese types of questions, mostly because it is easy to interpert.  


We can ask two interesting questions:

- Does adding text-related indicators outpreform a naive model?  

- Which indicators (or inteactions) are good predictors?  


This framework can easily be extended to test for predictability in other types of response variables (e.g., the exchange rate)
